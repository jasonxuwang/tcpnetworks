...
int32_t TCPSocket::initialize(int32_t send_buffer_size, int32_t recv_buffer_size) {
	...
	//why should we reset head and tail over here?
	this->m_recv_head = 0;
	this->m_recv_tail = 0;
	return success;
}
...
int32_t TCPSocket::recv_data() {
	...
	if (this->m_recv_tail == this->m_recv_head) {
		this->m_recv_head = 0;
		this->m_recv_tail = 0;
	}
	...
	do {
		...
		received_byte = recv(m_socket_fd, &m_recv_buffer[m_recv_tail], size_t(m_recv_buffer_size - m_recv_tail), 0);//receive message in byte so that we can 
...
//nt32_t TCPSocket::process_data(std::function<int32_t(const char*, const int32_t, int32_t)> callback) {
//   int ret = recv_data();
//
//   if (ret < 0) {
